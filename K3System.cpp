...
{
     std::vector<float>* floatVect = this->content(name);
     float* element = &((*floatVect)[n]);
     return *element;
     //return &((*floatVect)[n]);
}
...
void K3System::char2floatVect(const char* name)
{
     struct Freedom* f = this->node(name);
     //this->reset(f);
     //std::vector<float> floatVect = f->valeur;
